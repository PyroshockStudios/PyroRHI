...
TEST_F(RHI_CONTEXT_FIXTURE_NAME, CommandsTransferCopyBufferToBufferSucceeds) {
    ...
    cb->Complete();
    mDevice->SubmitQueue({ .queue = mDevice->GetCommandQueues()[0], .commands = { &cb, 1 } }); // queue is a pointer/handle
    mDevice->WaitIdle();
...
TEST_F(RHI_CONTEXT_FIXTURE_NAME, CommandsTransferCopyBufferToImageSucceeds) {
    ...
    auto requirements = mDevice->ImageUploadRequirements(image, ImageSlice{ .mipLevel = 0, .arrayLayer = 0 });
    ...
    mDevice->SubmitQueue({ .queue = mDevice->GetCommandQueues()[0], .commands = { &cb, 1 } });
    mDevice->WaitIdle();
...
TEST_F(RHI_CONTEXT_FIXTURE_NAME, MultiCommandBufferSyncSubmit) {
    ...
    eastl::array commands = { cbCopy, cbClear };
    mDevice->SubmitQueue({ .queue = queue, .commands = commands });
...
TEST_F(RHI_CONTEXT_FIXTURE_NAME, MultiThreadedCommandBufferRecordingSubmitOrder) {
    ...
    eastl::array submits = { cbCopy, cbClear };
    ...
    mDevice->SubmitQueue({ .queue = queue, .commands = submits });
...
TEST_F(RHI_CONTEXT_FIXTURE_NAME, CommandsBlitImageArray) {
    ...
    mDevice->SubmitQueue({ .queue = q, .commands = { &graphicsCommands, 1 } });
...
TEST_F(RHI_CONTEXT_FIXTURE_NAME, CommandsSuccessfullyDoRayQueryCSH) {
    ...
    readbackBufferInfo.size = PYRO_ALIGN(4, mDevice->Properties().minStorageBufferOffsetAlignment);
    readbackBufferInfo.usage = BufferUsageFlagBits::TRANSFER_DST;
    readbackBufferInfo.initialLayout = BufferLayout::TransferDst;
    readbackBufferInfo.allocationDomain = MemoryAllocationDomain::HostReadback;
    TRACK_RHI_PARAMETER(readbackBufferInfo);
    Buffer readbackBuffer = mDevice->CreateBuffer(readbackBufferInfo);
    TRACK_RHI_HANDLE(readbackBuffer);

    cmd = queue->GetCommandBuffer({ .name = "Run RT" });
    cmd->SetComputePipeline(pipeline);
    cmd->PushConstant(tlasId.index);
    cmd->SetUnorderedAccessView({
        .slot = 0,
        .view = uav,
    ...
    EXPECT_NO_FATAL_FAILURE(cmd->BufferBarrier({
        .buffer = buffer,
    ...
    // Copy a subregion with offsets
    CopyBufferToBufferInfo copyReadbackInfo{};
    copyReadbackInfo.srcBuffer = buffer;
    copyReadbackInfo.dstBuffer = readbackBuffer;
    copyReadbackInfo.size = bufferInfo.size;
    TRACK_RHI_PARAMETER(copyReadbackInfo);
    ...
    mDevice->Destroy(readbackBuffer);
}